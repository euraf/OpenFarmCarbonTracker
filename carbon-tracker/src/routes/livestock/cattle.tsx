import {
  DEFAULT_CATTLE_PRODUCTION_CONFIG,
  setStore,
  store,
} from "~/store/store";
          ...
          <div class="flex gap-2 items-center">
            ...
            <TextField class="flex-1">
              ...
              <TextFieldInput
                ...
                value={item.label}
                onInput={(e) =>
                  handleInputChange(index(), "label", e.currentTarget.value)
                }
              />
            </TextField>
...
function CattleProductionSection() {
  function handleInputChange(
    ...
    if (section === "year") {
      ...
      const prevYear =
        configIndex > 0 ? configs[configIndex - 1].year : -Infinity;
      const nextYear =
        configIndex < configs.length - 1
          ? configs[configIndex + 1].year
          : Infinity;
  ...
  function removeConfig(index: number) {
    setStore("livestock", "cattle", "production", "configurations", (configs) =>
              ...
              <div class="border-2 rounded-lg p-4 bg-white min-w-[320px] relative">
                <button
                  ...
                  class={`absolute top-2 right-2 ${
                    index() !== 0
                      ? "hover:bg-red-500 bg-gray-700"
                      : "bg-gray-400"
                  } hover:text-white text-white p-1 rounded-md flex items-center justify-center w-8 h-8`}
  ...
  return (
    <div class="emission-factors">
      <h3 class="text-lg font-semibold mb-4">
        Emission Factors (kg CO2e per animal)
      </h3>
      <a
        target="_blank"
        class="text-blue-500 block mb-2"
        href="https://github.com/euraf/OpenFarmCarbonTracker/blob/78d17ca0991d09de98cde370b96272c52f0b1cd5/carbon-tracker/src/data/livestock/emission-factors.ts#L9-L13"
      >
        Update sources
      </a>
      ...
      <div class="grid gap-4 mb-6">
        <EmissionFactorField
          label="Dairy Cows"
          value={() =>
            store.livestock.cattle.production.configurations[0].config.dairyCows
              .emissionFactor
          }
          defaultValue={DEFAULT_CATTLE_EMISSION_FACTORS.dairyCows}
          onChange={(value) => handleFactorChange("dairyCows", value)}
        />
        <EmissionFactorField
          label="Bulls"
          value={() =>
            store.livestock.cattle.production.configurations[0].config.bulls
              .emissionFactor
          }
          defaultValue={DEFAULT_CATTLE_EMISSION_FACTORS.bulls}
          onChange={(value) => handleFactorChange("bulls", value)}
        />
        <EmissionFactorField
          label="Meat Cattle"
          value={() =>
            store.livestock.cattle.production.configurations[0].config
              .meatCattle.emissionFactor
          }
          defaultValue={DEFAULT_CATTLE_EMISSION_FACTORS.meatCattle}
          onChange={(value) => handleFactorChange("meatCattle", value)}
        />
      </div>
    </div>
  );
